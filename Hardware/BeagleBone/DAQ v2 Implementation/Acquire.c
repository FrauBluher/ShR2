...
int process_serial(int serial_fd,char *output) {
	...
	ioctl(serial_fd, FIONREAD, &bytes); // Number of bytes in buffer.

	n = read(serial_fd, tmpBuff, bytes); // Reads data into buffer.
